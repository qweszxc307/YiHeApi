...
public class RecommendPayController extends SuperController {
    ...
    @ApiOperation(value = "分享返礼-赠送产品请求支付接口")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "分享领取订单id",  paramType = "path"),
            @ApiImplicitParam(name = "payType", value = "支付类型(0:微信支付,1:微信支付)", paramType = "body")
    })
    @PostMapping(value = "/wxSendPay/{id}")
    public ApiResponses<JSONObject> wxSendPay(@PathVariable("id") Integer currentOrderId,@RequestBody Integer payType) {
        try {
            String openId = JWTUtils.getOpenId(getToken());
            ...
            if (payType.equals(PayTypeEnum.YE.value())) {
                ...
                packageParams.put("total_fee", 1 + "");//支付金额，这边需要转成字符串类型，否则后面的签名会失败
                packageParams.put("spbill_create_ip", spbill_create_ip);
                packageParams.put("notify_url", WxApiEnum.WX_RECOMMEND_SEND_BACK_API.value());//支付成功后的回调地址
                packageParams.put("trade_type", TRADETYPE);//支付方式
                packageParams.put("openid", JWTUtils.getOpenId(getToken()) + "");//用户的openID，自己获取
                ...
                String xml = "<xml>" + "<appid>" + appId + "</appid>"
                        ...
                        + "<mch_id>" + mchId + "</mch_id>"
                        + "<nonce_str>" + nonce_str + "</nonce_str>"
                        + "<notify_url>" + WxApiEnum.WX_RECOMMEND_SEND_BACK_API.value() + "</notify_url>"
                        + "<openid>" + JWTUtils.getOpenId(getToken()) + "</openid>"
    ...
    //这里分享产品支付回调接口，微信支付成功后会自动调用
    @Resources(auth = AuthTypeEnum.OPEN)
    @PostMapping(value = "/wxRecommendBack")
    public void wxRecommendBack(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    //这里是领取商品支付回调接口，微信支付成功后会自动调用
    @Resources(auth = AuthTypeEnum.OPEN)
    @PostMapping(value = "/wxSendBack")
    public void wxSendBack(HttpServletRequest request, HttpServletResponse response) throws Exception {
        BufferedReader br = new BufferedReader(new InputStreamReader(request.getInputStream()));
        String line = null;
        StringBuilder sb = new StringBuilder();
        while ((line = br.readLine()) != null) {
